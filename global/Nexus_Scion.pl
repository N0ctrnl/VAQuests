...
sub EVENT_SAY { 
  if ($text=~/Hail/i){
    quest::say("Hello $name. I do not have much time to chat. I must concentrate on the portal that is due to open soon. If you wish to [". quest::saylink("journey to Luclin") . "], tell me so."); 
  }
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 19720 => 1)) {
